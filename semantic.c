...
NOEUD creer(const char *nom, TYPE_IDENTIFIER type, CLASS classs,
            NOEUD suivant) {
...
void foncCallEnd() {
  ...
  if (g_noeudFonc->nbParam != g_nbParam)
    semanticerror(concat("wrong number of parameters in method call: ",
                         g_noeudFonc->nom));
  g_nbParam = 0;
}
...
void finFonction() {
  ...
  while (tmp_table != NULL) {
    if (tmp_table->classs == variable && !tmp_table->isUsed) {
      // semanticwarning(concat("declared variable is not used:
      // ",tmp_table->nom)); printf("%s",tmp_table->nom);
